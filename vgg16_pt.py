...
def main():
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('data_proportions.png', bbox_inches='tight')
    n_1 = np.count_nonzero(np.argmax(y_train, axis=1))
    print('Train data distribution: normal-{} pneumonia-{}'.format(len(y_train)-n_1, n_1))
    n_1 = np.count_nonzero(np.argmax(y_test, axis=1))
    print('Test data distribution: normal-{} pneumonia-{}'.format(len(y_test)-n_1, n_1))
    ...
    checkpoint = ModelCheckpoint(
        'vgg16/weights.epoch_{epoch:02d}.hdf5',
        monitor='val_loss', 
